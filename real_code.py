...
# since python date format does not support ordinal numbers (1st, 2nd, 3rd, etc.)
# we calculate that seperately and merge it in with .format()
DATE_SEARCH_FORMAT = r'%A %B {ordinal_day} %Y'

# oddly, strftime() doesn't have this built in. It's a bit of a pain because
# the teens are an exception, which means it's easier to list out the 7 exceptions
# between 1 and 31 then to implement the full rule.
ORDINAL_SUFFIXES = {1:"st", 2:"nd", 3:"rd", 21:"st", 22:"nd", 23:"rd", 31:"st"}

def format_date_for_search(date):
    """date is formated "Sunday, January 1st 2014" so that google will see the relevant parts."""
...
def google_for_today():
    """yields the URL of every result on the first page of a google search for today's date."""
